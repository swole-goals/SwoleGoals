...
for i in range(0, 1):
    try:
        nextButton = driver.find_element_by_css_selector(
            ".bb-flat-btn.bb-flat-btn--size-lg.bb-spinner-btn.js-ex-loadMore.ExLoadMore-btn").click()
    except:
        break
...
for i in range(len(exerciseUrls)):
    try:
        ...
        exerciseName[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[1]/div[1]/h2").get_attribute("innerHTML")
        exerciseName[i] = exerciseName[i].strip()
        types[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[2]/div[2]/ul/li[1]/a").get_attribute("innerHTML")
        types[i] = types[i].strip()
        muscleWorked[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[2]/div[2]/ul/li[2]/a").get_attribute("innerHTML")
        muscleWorked[i] = muscleWorked[i].strip()
        equipment[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[2]/div[2]/ul/li[3]/a").get_attribute("innerHTML")
        equipment[i] = equipment[i].strip()
        level[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[2]/div[2]/ul/li[4]").get_attribute("innerHTML")
        level[i] = level[i].replace('Level:', "")  # formatting
        level[i] = level[i].strip()
        rating[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[2]/div[3]/div/div[1]").get_attribute("innerHTML")
        rating[i] = rating[i].strip()
        imageOne[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[3]/div[1]/div[1]/img").get_attribute("src")
        imageTwo[i] = driver.find_element_by_xpath(
            "//*[@id='js-ex-content']/div/section[3]/div[1]/div[2]/img").get_attribute("src")
...
for i in range(len(exerciseUrls)):
    assert exerciseName[i] is not None
    assert types[i] is not None
    assert muscleWorked[i]is not None
    assert equipment[i] is not None
    assert level[i] is not None
    assert rating[i] is not None
    assert imageOne[i] is not None
    assert imageTwo[i] is not None
