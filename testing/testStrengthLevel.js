...
var testSelectNoFilters = function() {
	var cmd = "select * from strengthlevel";
...
var testSelectFilteredByName = function() {
	var cmd = "select exercise_name from strengthlevel";
	con.query(cmd, function (err, result) {
		assert(result.length > 0);
	});
...
var testSelectBadFilter = function() {
	var cmd = "select fake from strengthlevel";
	con.query(cmd, function (err, result) {
		assert(result == null);
	});
...
testExercisesContainPerformance();
